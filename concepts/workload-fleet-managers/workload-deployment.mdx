...
> Note: this is the recommendation from FluxCD [https://fluxcd.io/flux/guides/repository-structure/](https://fluxcd.io/flux/guides/repository-structure/)

- The workload orchestration solution MUST store the device's [desired state documents](../../margo-api-reference/margo-management-interface/desired-state.mdx) within a Git repository the device's management client can access. 
...
1. The workload orchestration solution creates the [desired state documents](../../margo-api-reference/margo-management-interface/desired-state.mdx) based on the end user's inputs when installing, updating or deleting an application.
...
2. While the new desired state is being applied, the device's management client MUST report progress on state changes (see the [deployment state](#deployment-status) section below) using the [Device API](../../margo-api-reference/margo-management-interface/deployment-status.mdx)

#### Deployment Status

The deployment status is sent to the workload orchestration web service using the [Device API](../../margo-api-reference/margo-management-interface/deployment-status.mdx) when there is a change in the deployment state. This informs the workload orchestration web service of the current state as the new desired state is applied. 
