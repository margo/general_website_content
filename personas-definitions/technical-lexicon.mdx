...
According the [specification](../concepts/workloads/application-package-definition.mdx), it is a folder in a Git repository referenced by URL, which contains the Application Description (that refers to contained and deployable Components) as well as associated resources (e.g., icons).
