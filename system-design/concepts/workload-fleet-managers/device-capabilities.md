...
The purpose of capability reporting is to ensure the Workload Fleet Management (WFM) solution has the information needed to pair workloads with compatible Target Compute. Capabilities are reported to the WFM web service using the [Device Capabilities API](../../specification/margo-management-interface/device-capabilities.md).
...
The Target Name describes the Target Compute surface itself and carries no security meaning; the identity a client authenticates with is issued through the [Margo Identity and Authorization Framework](../identity/identity-and-trust.md).
