...
In order to achieve Margo's interoperability mission statement, the Margo Management Interface is a critical component that enables vendor interoperability between Workload Fleet Management suppliers and Compute Suppliers. These suppliers are able to utilize the open implementation provided by the Margo community as is, or follow the specification to build their own compatible client/server components.
